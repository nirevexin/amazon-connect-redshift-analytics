...
-- ============================================================
-- Amazon Connect agent call metrics with shift analysis
-- Includes fixed shift periods (calls fully within 9-13, 13-17, after 17)
-- ============================================================
WITH base AS (
    SELECT
        agent_conn,
...
enriched AS (
    SELECT
        c.agent_conn,
        c.disconn_time,
        c.agent_interact_duration,
        c.agent_afw_duration,
        c.agent_longest_hold,
        c.agent_conn_att,
        c.customer_hold_duration,
        c.customer_phone,
        c.init_method,
        u.user_name,
        u.user_lastname,
    ...
    FROM base c
    LEFT JOIN connect.dim_users u ON c.agent_id = u.user_id
    LEFT JOIN connect.dim_queues q ON c.queue_id = q.queue_id
    LEFT JOIN litify.dim_users l ON c.agent_username = l.username
),
...
metrics AS (
    SELECT
        ...
        SUM(agent_interact_duration) / 60 AS total_agent_interaction_minutes,
        ...
        SUM(EXTRACT(EPOCH FROM disconn_time - agent_conn)) / 60 AS total_contact_duration_minutes,
        ...
        SUM(agent_afw_duration) / 60 AS total_agent_afw_duration_minutes,
        ...
        SUM(customer_hold_duration) / 60 AS total_customer_hold_duration_minutes,
        ...
        -- Duration buckets (using SUM with CASE)
        ...
        -- Calls that start AND end after 17:00 (excludes overnight calls)
        SUM(CASE
